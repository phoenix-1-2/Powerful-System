...
export default function TaskList({ tasks, onToggle, onDelete }) {
	const groupsMap = tasks.reduce((acc, t) => {
		...
		if (!acc[key]) acc[key] = { key, title: displayTitle, items: [], totalPoints: 0 };
	...
	const toggleGroup = (key) => setExpandedGroups((s) => ({ ...s, [key]: !s[key] }));

	return (
		<div className="rounded-xl border border-zinc-800 overflow-hidden bg-zinc-950">
			{groups.length === 0 ? (
				<div className="text-sm text-zinc-400 p-4">No tasks yet. Add one above.</div>
			) : (
				groups.map((g, gi) => {
					const isOpen = expandedGroups[g.key] ?? true;
					return (
						<div key={g.key} className={gi === 0 ? '' : 'border-t border-zinc-800'}>
							<button
							...
							>
								...
								<span className="text-sm text-zinc-300 flex items-center gap-2">
									<span className="px-2 py-0.5 rounded-full border border-zinc-700 bg-zinc-800 text-zinc-200 text-xs">{g.items.length}</span>
									{g.totalPoints} pts
								</span>
							</button>
							{isOpen && (
										...
										>
											<label className="flex items-center gap-3 cursor-pointer min-w-0">
												...
												<div className="min-w-0">
													<div className={`font-medium text-base flex items-center gap-2 ${t.completed ? 'text-fuchsia-300' : 'text-zinc-100'}`}>
														{t.completed && <CheckCircle2 className="text-fuchsia-500" size={18} />}
														<span className="truncate block">{t.title}</span>
													</div>
													<div className="text-xs text-zinc-400">{t.points} pts</div>
												</div>
											</label>
											<motion.button
