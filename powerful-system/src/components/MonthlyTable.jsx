...
export default function MonthlyTable({ monthlyTotals, dailyTarget = 100 }) {
	...
	if (months.length === 0)
		return <div className="rounded-xl border border-zinc-800 p-4 text-sm text-zinc-400 bg-zinc-950">No monthly data yet.</div>;
	return (
		<div className="rounded-xl border border-zinc-800 overflow-hidden bg-zinc-950 text-zinc-100">
			<table className="w-full text-sm">
				...
				<tbody>
					{months.map((m) => (
						<tr key={m} className="border-t border-zinc-800 hover:bg-zinc-900/70 transition">
								...
								<div className="flex flex-col gap-2">
									{Object.entries(monthlyTotals[m].weeks).map(([wk, total]) => (
										<div key={wk} className="flex items-center justify-between gap-3">
											...
											<span className="text-zinc-300">{total} pts</span>
										</div>
									))}
								</div>
							</td>
							<td className="p-3 text-right font-semibold">{monthlyTotals[m].total} pts</td>
